...
const ProjectCard: React.FC<{ project: Project }> = ({ project }) => {
  ...
  return (<div
    className="group w-full h-full relative  sm:aspect-square select-none cursor-pointer duration-300 sm:hover:-translate-y-2 min-h-[400px]" >
    <div
      ...
      <div className="flex gap-2  mt-auto ml-auto">
        {repo && <a style={{ color: colorTopLeft.hex }} href={liveSite} target='_blank' rel='noreferrer noopener' className="rounded-full sm:opacity-0 text-3xl p-2 bg-white relative duration-300 group-hover:opacity-100 scale-95 group-hover:scale-100 hover:!scale-105 active:!scale-95">
          <RiGithubFill />
        </a>}
        {liveSite && <a style={{ color: colorTopLeft.hex }} href={liveSite} className="rounded-full sm:opacity-0 text-3xl p-2 bg-white relative duration-300 group-hover:opacity-100 scale-95 group-hover:scale-100 hover:!scale-105 active:!scale-95">
          <HiEye />
        </a>}
      </div>
    </div>
